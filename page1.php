...
.page-badge{font-family:'Montserrat',sans-serif;font-weight:900;font-size:2.2rem;color:var(--violet);line-height:1;}
...
<div class="wrap">
  ...
  <div class="page-header">
    <div>
      ...
      <div class="page-title">Page <span style="color:var(--violet)"><?= $page_num ?></span> — Most Recent Mail</div>
    </div>
  </div>
  ...
  <div class="email-card">
    ...
    <table class="info-table">
      <tr>
        <td>Sender</td>
        <td class="val-primary"><?= htmlspecialchars($email['from_name'] ?: '—') ?>
          <?php if ($email['from_email'] && $email['from_email'] !== $email['from_name']): ?>
          <br><span style="font-size:.78rem;color:var(--text-3);font-weight:400"><?= htmlspecialchars($email['from_email']) ?></span>
          <?php endif; ?>
        </td>
      </tr>
      <tr>
      ...
      <tr>
        <td>Subject</td>
        <td class="val-primary"><?= htmlspecialchars($email['subject'] ?? '(No Subject)') ?></td>
      </tr>
      <tr>
        <td>Directory</td>
        <td><span style="background:var(--violet);color:var(--text);font-size:.72rem;font-weight:700;padding:3px 12px;border-radius:6px;border:1px solid rgba(235,244,221,.25)">Page <?= $page_num ?></span></td>
      </tr>
      <tr>
        <td>Date</td>
        <td><?= $email['received_at'] ? date('D, M j Y  g:i a', strtotime($email['received_at'])) : '—' ?></td>
      </tr>
      <tr>
        <td>Folder</td>
        <td><?= ucfirst(htmlspecialchars($email['folder'] ?? 'inbox')) ?></td>
      </tr>
      <tr>
        <td>Body</td>
        <td style="color:var(--text-2);white-space:pre-wrap;max-height:320px;overflow-y:auto"><?= htmlspecialchars(trim(strip_tags($email['body_plain'] ?? '(No body)'))) ?></td>
      </tr>
    </table>
    <?php else: ?>
